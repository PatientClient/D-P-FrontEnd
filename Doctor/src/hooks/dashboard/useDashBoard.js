...
export const useDashBoard=()=>{
  ...
  const { data: users, setData: setUsers ,request:refreshUsers} = useApi('/user')
  const { request: updateUser } = useApiRequest()
  ...
  const updateUserStatus = async (user) => {
    ...
    const res = await updateUser(`/user/${user.userId}`, 'PATCH', { status: user.status })
    console.log(res);
    if(res&& !res.error){
      refreshUsers()
    }
